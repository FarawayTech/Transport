...
class DB {
    ...
    private static function getMinDistanceFromCursor($cursor, $lon, $lat, $limit) {
    ...
    private static function processQuery($query) {
        ...
        return preg_split('/\s+/', strtolower($query));
    }
    ...
    public static function findNearbyLocationsQuery($query, $lon, $lat, $limit, $config) {
        ...
        $full_cursor = $collection->find(Array('names'=>Array('$all'=> $query)));
        $prefix_cursor = $collection->find(Array('prefix_names'=>Array('$all'=> $query),
                                                 'names'=>Array('$not'=>Array('$all'=> $query))));
        $full_count = $full_cursor->count();
        $stations = array();

        if ($full_count < $limit) {
            if ($full_count > 0) {
                $stations = self::sortByDistance(LocationFactory::createFromMongoCursor($full_cursor, $lon, $lat));
            ...
            $result_cursor = $collection->find(Array('location' => Array('$nearSphere' =>
                ...
                'names'=>Array('$all'=>$query)))->limit(intval($limit));
            return LocationFactory::createFromMongoCursor($result_cursor, $lon, $lat);
        }
        else {
            // sort $full_cursor by distance and get limit
            return self::getMinDistanceFromCursor($full_cursor, $lon, $lat, $limit);
        }

        // full count was less than limit or 0, get prefix count
        $prefix_count = $prefix_cursor->count();
        if ($prefix_count < $limit) {
            return array_merge($stations, self::sortByDistance(LocationFactory::createFromMongoCursor($prefix_cursor, $lon, $lat)));
        }
        else if ($count*$limit/$prefix_count < $prefix_count) {
            $result_cursor = $collection->find(Array('location' => Array('$nearSphere' =>
                Array('$geometry' => Array('type'=>'Point', 'coordinates' => Array(floatval($lon), floatval($lat))))),
                'prefix_names'=>Array('$all'=>$query), 'names'=>Array('$not'=>Array('$all'=> $query))))->limit(intval($limit));
            return array_merge($stations, LocationFactory::createFromMongoCursor($result_cursor, $lon, $lat));
        }
        else {
